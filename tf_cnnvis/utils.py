...
def _write_activation(activations, layer, path_outdir, path_logdir, name_suffix=''):
    is_success = True
    ...
    path_log = path_logdir
    is_success = make_dir(path_log)

    with tf.Graph().as_default() as g:
        with tf.variable_scope(layer.lower().replace("/", "_")):
            image = tf.placeholder(tf.float32, shape = [None, None, None, None])
            name = 'All_At_Once_Activations'
            if name_suffix:
                name += '_' + name_suffix
            image_summary_t = tf.summary.image(name = name, tensor = image, max_outputs = config["MAX_IMAGES"])
    ...
    return is_success
def _write_deconv(images, layer, path_outdir, path_logdir, name_suffix=''):
    is_success = True
    ...
    with tf.Graph().as_default() as g:
        with tf.variable_scope(layer.lower().replace("/", "_")):
            ...
            name = 'All_At_Once_Deconv'
            if name_suffix:
                name += '_' + name_suffix
            #image_summary_t1 = tf.summary.image(name = "One_By_One_Deconv", tensor = image, max_outputs = config["MAX_FEATUREMAP"])
            image_summary_t2 = tf.summary.image(name = name, tensor = image, max_outputs = config["MAX_IMAGES"])
...
######################################
def _write_deepdream(images, layer, path_outdir, path_logdir, name_suffix=''):
    is_success = True
    ...
    with tf.Graph().as_default() as g:
        with tf.variable_scope(layer.lower().replace("/", "_")):
            ...
            name = 'One_By_One_DeepDream'
            if name_suffix:
                name += '_' + name_suffix
            image_summary_t = tf.summary.image(name = name, tensor = image, max_outputs = config["MAX_FEATUREMAP"])
        ...
        try:
            ...
            file_writer.close() # close file writer
    return is_success
def write_results(results, layer, path_outdir, path_logdir, method, name_suffix=''):
    is_success = True

    if method == "act":
        is_success = _write_activation(results, layer, path_outdir, path_logdir, name_suffix=name_suffix)
    elif method == "deconv":
        is_success = _write_deconv(results, layer, path_outdir, path_logdir, name_suffix=name_suffix)
    elif method == "deepdream":
        is_success = _write_deepdream(results, layer, path_outdir, path_logdir, name_suffix=name_suffix)
    return is_success
...
def _im_normlize(images, ubound = 255.0):
    ...
    if len(images[0].shape) == 2:
        for i in range(N):
            images[i] = images[i].reshape([images[0].shape[0], 28, 28, 1])
